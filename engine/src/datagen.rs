...
fn play_game(
    ...
    loop {
        ...
        let eval = result.score.clamp(-10000, 10000) as i16;

        // Skip positions where eval is mate-like (terminal soon)
        if eval.abs() < 9000 {
    ...
    let result_byte = match game_result {
        Some(GameResult::Win(Side::White)) => 2u8,
        Some(GameResult::Win(Side::Black)) => 0u8,
        Some(GameResult::Draw) => 1u8,
        None => 1u8, // shouldn't happen
    };
...
/// Run data generation
pub fn run_datagen(num_games: u32, depth: i32, time_ms: u64, num_threads: u32) {
    ...
    for thread_id in 0..num_threads {
        ...
        handles.push(std::thread::spawn(move || {
            let filename = format!("data_thread_{}.bin", thread_id);
            ...
            for _ in 0..games_per_thread {
                ...
                let records = play_game(&mut searcher, depth, time_ms, &mut rng);
                ...
                if done % 100 == 0 {
                    eprintln!(
                        "Games: {}/{}, positions: ~{}, ({:.0} games/min)",
                        done,
                        num_games,
                        positions_total.load(Ordering::Relaxed),
                        done as f64 / start_time.elapsed().as_secs_f64() * 60.0,
                    );
                }
            }
    ...
    let output_file = "training_data.bin";
    {
